...
interface CreateBookingLinkRequest {
  ...
  duration?: number  // Override duration in minutes (otherwise uses availability settings)
  expiryDays?: number  // Default: 3 days
  maxUses?: number     // Default: 1
  notes?: string       // Optional notes for the candidate
}
      ...
      try {
        ...
        if (availabilityDoc.exists) {
          ...
          availabilitySettings = {
            ...
            slots: avail?.slots || [],
            blockedDates: avail?.blockedDates?.map((d: any) => 
...
export const sendBookingConfirmation = onCall(
  ...
  async (request) => {
    ...
    const icsContent = generateICSFile(
      title,
      `Your ${typeLabel.toLowerCase()} has been scheduled.\\n\\nCandidate: ${candidateName}\\nPosition: ${jobTitle || 'Not specified'}\\nLocation: ${location}\\nDuration: ${duration} minutes`,
      location,
      scheduledDate,
      endDate
    )
